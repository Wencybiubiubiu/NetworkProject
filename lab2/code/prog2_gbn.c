...
int A_timerinterrupt() {
    ...
    if(cur_head->wait_ACK == WAITING_ACK){
        ...
        //printf("==========CHECK:%d++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n",cur_head->cur_package.seqnum);
        struct info * temp = cur_head;
        printf("Total number of packet in window is %d: \n",num_of_cur_packet);
        for(int i = 0; i < num_of_cur_packet; i ++){
            stoptimer(temp->cur_package.seqnum);
            starttimer(temp->cur_package.seqnum,timer_interval);
            printf("-Resend packet with seqnum: %d, acknum: %d\n",temp->cur_package.seqnum,temp->cur_package.acknum);
...
int B_input(struct pkt packet)
{
    ...
    if(expected_seqnum != packet.seqnum){
        printf("-Expected seqnum is %d. need resends ack,  Current seqnum: %d.\n", expected_seqnum, packet.seqnum);
