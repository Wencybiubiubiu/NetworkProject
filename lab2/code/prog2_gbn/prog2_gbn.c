...
int A_output(struct msg message)
{
    ...
    printf("-Packet sent.\n");
    printf("-Current state: seqnum is %d, acknum is %d. Check cur_package: %d, %d\n", A_side->SEQ, A_side->ACK, cur_head->cur_package.seqnum, cur_head->cur_package.acknum);
...
int A_input(struct pkt packet)
{
    ...
    if(num_of_msg_in_buffer > 0){
        printf("-There are/is some message in buffer, move the first one to window.\n");
    ...
    if(num_of_cur_packet >= 1){
        new_interrupt = cur_head->next->end_time;
    }
    ...
    if(num_of_cur_packet >= 1){
        printf("-Adjust timer with new_interrupt: %f, current time: %f,seqnum: %d, interrupt time: %f", new_interrupt, time, cur_head->cur_package.seqnum,cur_head->end_time);
...
int A_timerinterrupt() {
    ...
    if(cur_head->wait_ACK == WAITING_ACK){
        ...
        starttimer(A,timer_interval);
        printf("Total number of packet in window is %d: ",num_of_cur_packet);
        for(int i = 0; i < num_of_cur_packet; i ++){
            ...
            temp->end_time = time + timer_interval;
            temp = temp->next;
        }
        return 0;
    }
...
int B_input(struct pkt packet)
{
    ...
    if(expected_seqnum != packet.seqnum){
        printf("-Expected seqnum is greater. need resends ack,  Current seqnum: %d.\n", packet.seqnum);
        tolayer3(B,B_side->cur_package);
        printf("-Receiver side send acked number by packet back to sender. acknum: %d.\n", B_side->cur_package.acknum);
        return 0;
    }
...
void starttimer(int AorB, float increment)
{
  ...
  //evptr->pktptr = &(A_side->cur_package);
